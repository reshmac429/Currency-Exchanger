...
let apikey='f84e9843c4971ae2d6cfc81b';
...
for (let i = 0; i < drop.length; i++) {
    for(currcode in countryList){
        ...
        drop[i].insertAdjacentHTML("beforeend",optionTag);
    }
...
function getExchangeRate(){
    ...
    exchangeText=document.querySelector(".msg");
    ...
    fetch(url).then(response => response.json()).then(result =>{
        ...
        exchangeText.innerHTML=`${amountval} ${fromcurr.value}=${totalExchange}${tocurr.value}`;

        // exchangeText.innertext=`${amountval} ${fromcurr.value}=${totalExchange}${tocurr.value}`;
    });
}